 ...
 * 在屏幕上画出的游戏面板，然后调用玩家和敌人对象的 update / render 函数（在 app.js 中定义的）
 *
 * 一个游戏引擎的工作过程就是不停的绘制整个游戏屏幕。
 * 当玩家在屏幕上移动的时候，看上去就是图片在移动或者被重绘。
 * 但这都是表面现象。实际上是整个屏幕被重绘导致这样的动画产生的假象。

 * 这个引擎是可以通过 Engine 变量公开访问的，
 * 而且它也让 canvas context (ctx) 对象也可以公开访问，以此使编写app.js的时候更加容易
 */
